        ...
        public override void OnUpdate()
        {
            if (BasePlayer.LocalPlayer?.CanInteract() == true)
            {
                ...
                for (var i = 0; i < BasePlayer.ListPlayers.Count; i++)
                {
                    ...
                    string text = $"<size=12>{BasePlayer.ListPlayers[i].Username} [{(int) BasePlayer.ListPlayers[i].Health} hp] {(int) Vector3.Distance(BasePlayer.LocalPlayer.Position, BasePlayer.ListPlayers[i].Position)}m</size>";
